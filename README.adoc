...
Any change to the authentication logic will require 50 updates.

Middleware's Limitation: It performs necessary preparation, but generally allows the request to continue to the handler unless configured with highly specific, hard-to-maintain logic. It focuses on state management, not access enforcement.
...
This example centralizes the security logic using a custom `Extractor` (for authentication) and an `Identity Extractor Guard` (for role-based access control), which is a powerful and clean design pattern for scalable applications.

- Authentication Extractor (`AuthUser`): Ensures a user is logged in (`HTTP 401` if not).
- Role Guard Extractor (`AdminUser`): Builds on top of `AuthUser` and enforces a specific role (`HTTP 403` if not an `admin`).
