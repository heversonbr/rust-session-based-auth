...
In summary, the `Middleware` sets the stage by handling the session mechanics, while the `Extractor` steps in as the gatekeeper to enforce the security policy defined for each specific route. As a next step, we could start thinking about improving the security by encrypting the credentials and using a real database to store credentials.